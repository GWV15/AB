...
def heuristicCost(field, path, goal_pos):
    ...
    if len(p_list) > 1:
        ...
        nearest_P_Goal = p_list[np.argmin([getManhattanDist(node, goal_pos)
                                for node in p_list])]
        return len(path) + min(getManhattanDist(path[-1], goal_pos),
                               getManhattanDist(path[-1], nearest_P_Start) +
                               getManhattanDist(nearest_P_Goal, goal_pos))
...
# _dataStructure    - data structure for the frontier (decides which
#                     search alg. to use)
# _heuristic        - flag: use heuristics
# _debug            - flag: use debugging method
def genericSearch(field, start_pos_list, end_pos_list,
                  _dataStructure=Queue, _heuristic=False, _debug=False):
    # Init
    visited = []
            ...
            for neighbor in getNeighbors(field, head):
                ...
                if not isBound(field,neighbor):
                    ...
                    if not _heuristic:
                        ...
                        frontier.put((heuristicCost(field, new_path,
                                      end_pos_list[0]), new_path))
...
# following_question    - if the cli-arg or input of first_question
#                           is not in answers, ask this question
# answers               - list of possible answers
def askForAnswer(cli_arg, first_question, following_question,
                 answers=["y", "n"]):
...
def runSearch(field,algorithm,debug): # Run the search
    ...
    if algorithm == "astar":
        ...
        return genericSearch(field, start_list, goal_list,
                             _dataStructure=LifoQueue, _heuristic=False,
                             _debug=debug)
    elif algorithm == "bfs":
        return genericSearch(field, start_list, goal_list,
                             _dataStructure=Queue, _heuristic=False,
                             _debug=debug)
...
#
# CLI usage:
# If all arguments are given the search does not rely on
# any further user interaction
#
# If a given cli-arg does not match the answers,
# the programm asks interactively
#
